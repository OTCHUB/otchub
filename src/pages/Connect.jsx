...
// MCP server URL resolved from the current origin — never hardcoded.
const MCP_URL = new URL("/api/mcp", window.location.origin).toString();
const LLMS_URL = new URL("/llms.txt", window.location.origin).toString();
...
const AGENT_PROMPT = `You are connected to OTC_HUB — the MCP data server of OTC Pulse Solana, a community-built (unofficial) analytics dashboard for the OTC_DESK protocol on Solana (official app: otcdesks.cash).
